...
-- me (QwertyL0L) for putting it all together (and yes ik my code prolly looks like dogshit but im still new to lua so ye)
...
local Window = Rayfield:CreateWindow({
	Name = "VC Flinger",
	Icon = 0,
	LoadingTitle = "VC Flinger",
	LoadingSubtitle = "Created by QwertyL0L.",
	Theme = "Default",
	DisableRayfieldPrompts = false,
	DisableBuildWarnings = false,
	ToggleUIKeybind = Enum.KeyCode.Period
})

local MainTab = Window:CreateTab("Main", 4483362458)
MainTab:CreateSection("Might be a little buggy sometimes but it still works for the most part.")
...
local function fling()
	...
	task.spawn(function()
		local elapsed = 0
		while elapsed < tpDuration and toggled do
        ...
        if analyzer.RmsLevel > 0 and not flungRecently then
            ...
            if rootPart and targetRoot then
                ...
                followConn = RunService.RenderStepped:Connect(function()
                    ...
                    if elapsed > tpDuration or not toggled then
...
if canFlingPlayers() then
    Rayfield:Notify({
        Title = "You can fling other players!",
        Content = "Press " .. ToggleKey.Name .. " to start the script!",
        Duration = 3,
        Image = "check-circle-2",
    })
else
    Rayfield:Notify({
        Title = "You can't fling other players. Cancelling script...",
        Content = "TIP: Next time join a game with collision on for players.",
        Duration = 3,
        Image = "circle-x",
    })
    return
...
local MainToggle = MainTab:CreateToggle({
    ...
    Callback = function(Toggled)
        if isManualToggle then return end

        toggled = Toggled
        if toggled then
            Rayfield:Notify({
                Title = "VC Flinger On!",
                Content = "VC Flinger is now on!",
                Duration = 6.5,
                Image = "check-circle-2",
            })
            startTracking()
            ...
            Rayfield:Notify({
                Title = "VC Flinger Off!",
                Content = "VC Flinger is now off!",
                Duration = 6.5,
                Image = "check-circle-2",
            })
            stopTracking()
...
local PlayerDropdown = MainTab:CreateDropdown({
    ...
    Callback = function(Option)
        targetName = typeof(Option) == "table" and Option[1] or tostring(Option)
        Rayfield:Notify({
            Title = "VC Flinger",
            Content = "Target set to: " .. targetName,
            Duration = 3
        })
    end
})
...
local function refreshDropdown()
    ...
    Rayfield:Notify({
        ...
        Content = "Dropdown has been updated!",
        Duration = 3
    })
end
...
UIS.InputBegan:Connect(function(input)
    if input.KeyCode == ToggleKey and not UIS:GetFocusedTextBox() then
        toggled = not toggled
        isManualToggle = true
        MainToggle:Set(toggled)
